...
import com.KristiaN1337.vcast.wrappers.ContentProvider;
import com.KristiaN1337.vcast.wrappers.ServiceManager;

import java.io.File;
import java.io.IOException;

/**
 * Handle the cleanup of vcast, even if the main process is killed.
 * <p>
 * This is useful to restore some state when vcast is closed, even on device disconnection (which kills the vcast process).
 */
public final class CleanUp {

    public static final String SERVER_PATH = "/data/local/tmp/vcast-server.jar";
    ...
    public static void configure(boolean disableShowTouches, int restoreStayOn) throws IOException {
        ...
        if (needProcess) {
            ...
            // There is no additional clean up to do when vcast dies
            unlinkSelf();
        }
    }
