...
#define SOCKET_NAME "vcast"
#define SERVER_FILENAME "vcast-server"

#define DEFAULT_SERVER_PATH PREFIX "/share/vcast/" SERVER_FILENAME
#define DEVICE_SERVER_PATH "/data/local/tmp/vcast-server.jar"

static char *
get_server_path(void) {
#ifdef __WINDOWS__
    const wchar_t *server_path_env = _wgetenv(L"vcast_SERVER_PATH");
#else
    const char *server_path_env = getenv("vcast_SERVER_PATH");
#endif
    if (server_path_env) {
        ...
        LOGD("Using vcast_SERVER_PATH: %s", server_path);
        return server_path;
    }
    ...
    // use vcast-server in the same directory as the executable
