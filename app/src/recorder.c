...
static const AVRational vcast_TIME_BASE = {1, 1000000}; // timestamps in us

static const AVOutputFormat *
find_muxer(const char *name) {
#ifdef vcast_LAVF_HAS_NEW_MUXER_ITERATOR_API
    void *opaque = NULL;
#endif
...
record_packet_new(const AVPacket *packet) {
    ...
    // See <https://github.com/KristiaN1337/vcast/issues/707>
...
recorder_open(struct recorder *recorder, const AVCodec *input_codec) {
    ...
    av_dict_set(&recorder->ctx->metadata, "comment",
                "Recorded by vcast " vcast_VERSION, 0);
...
#ifdef vcast_LAVF_HAS_NEW_CODEC_PARAMS_API
...
recorder_rescale_packet(struct recorder *recorder, AVPacket *packet) {
    AVStream *ostream = recorder->ctx->streams[0];
    av_packet_rescale_ts(packet, vcast_TIME_BASE, ostream->time_base);
}
