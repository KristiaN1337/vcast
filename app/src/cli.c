...
# define CTRL_OR_CMD "Ctrl"
    ...
    fprintf(stderr,
        ...
        "\n"
        "    --always-on-top\n"
        "        Make vcast window always on top (above other windows).\n"
        "\n"
        "    -b, --bit-rate value\n"
        ...
        "        By default, to minimize latency, vcast always renders the\n"
        ...
        "        on exit.\n"
        "        It only shows physical touches (not clicks from vcast).\n"
        "\n"
        "    -v, --version\n"
        "        Print the version of vcast.\n"
        "\n"
        "    -V, --verbosity value\n"
        ...
        "\n"
        "    -w, --stay-awake\n"
        "        Keep the device on while vcast is running, when the device\n"
        "        is plugged in.\n"
        "\n"
        "    --window-borderless\n"
...
vcast_parse_args(struct vcast_cli_args *args, int argc, char *argv[]) {
    ...
    struct vcast_options *opts = &args->opts;
